# -*- coding: utf-8 -*-
"""
Created on Sun Feb 14 23:16:39 2021

@author: brand
"""

def Class InceptionBlock(nn.Module):
    ...
    def __init__(self, in_ch,out_ch):
        ...
        self.convolve3 = nn.Conv2d(in_ch,out_ch, kernel_size= 3, stride=1, padding=1)
        self.convolve5 = nn.Conv2d(in_ch, out_ch, kernel_size=5, stride=1, padding=2 )
        self.max = nn.MaxPool2d(3,stride=1,padding='same')
    ...
    def forward(self,x):
        ...
        y2 = self.bn(self.relu(self.convolve3(x)))
        y3 = self.bn(self.relu(self.convolve5(x)))
        ymax = self.bn(self.relu(self.convolve1(self.max(x))))
        
        inception = torch.cat([y1,y2], dim=1)
        inception = torch.cat([inception,y3], dim=1)   
        inception = torch.cat([inception,ymax], dim=1) 
        return inception

def Class BrandNet(nn.Module):
    ...
    def __init__(self, n_class):
        ...
        self.inception1 = nn.InceptionBlock(64,16)
        self.inception2 = nn.InceptionBlock(128,32)
        self.inception3 = nn.InceptionBlock(256,64)
        self.inception4 = nn.InceptionBlock(512,128)
